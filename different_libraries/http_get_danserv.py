            ...
            if not data or http_request_already_sent:
                ...
                if req_parsed.valid:
                    ...
                    if "favicon" in req_parsed.path:
                        ...
                        output_response = make_response(url_path=req_parsed.path, is_favicon=True)
                    ...
                    LOGGER.info(f"[Server,{HOST}:{PORT}][{thread_name}] incorrect request data "
                                f"(remote endpoint: {c_address[0]}:{c_address[1]}).")
                    output_response = make_response(url_path=req_parsed.path, incorrect_request=True, http_method=req_parsed.method)
                    c_socket.sendall(output_response)
                    return None

    # LOGGER.debug(f"[Server,{HOST}:{PORT}][{thread_name}] exiting socket context manager with closing the connection (remote endpoint: {c_address[0]}:{c_address[1]}).")
